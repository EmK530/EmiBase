        ...
        switch(obj_type)
        {
            ...
            */
            case EObjectType_EText:
            {
                ...
                text->BackgroundColor = Color32_deserialize(reader);
                text->TextColor = Color32_deserialize(reader);
                uint32_t textLen = BR_ReadU32(reader);
                char* str = BR_ReadString(reader, textLen);
                EText_SetTextN(text, str, textLen);
                MemFree(str);
